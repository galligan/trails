...
// Initialize database
let db: FieldbooksDb | null = null;

async function initializeDb(): Promise<void> {
  db = await setupFieldbook();
}
  ...
  switch (name) {
    case 'addEntry': {
      try {
        ...
        if (error instanceof FieldbooksDbError) {
...
server.setRequestHandler(ListToolsRequestSchema, () => {
  return {
    tools: [
      ...
      {
        name: 'listEntries',
        description: 'List entries from the fieldbook',
        inputSchema: {
          type: 'object',
          properties: {
            authorId: {
...
async function runServer(): Promise<void> {
  ...
  console.error('Fieldbooks MCP server running on stdio');
}
