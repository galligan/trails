...
export async function loadConfig(cwd = process.cwd()) {
    ...
    try {
        const parsedConfig = LogbookConfigSchema.parse(projectConfigResult.config);
        return {
            config: parsedConfig,
        ...
        if (error instanceof z.ZodError) {
            ...
            throw new Error(`Configuration file validation failed at ${projectConfigResult.filepath}:\n${issues.join('\n')}`);
        }
        throw error;
    }
}
