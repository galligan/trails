...
program
  .name('fieldbooks')
  .description('CLI for Fieldbooks field recording service')
  .version('0.0.1');

program
  .command('init')
  .description('Initialize a new fieldbook in the current directory')
...
program
  ...
  .option('-a, --author <id>', 'Author ID', process.env.FIELDBOOKS_AUTHOR_ID)
  ...
  .action((markdown: string, options: { author?: string; timestamp?: number; type?: string }) => {
    if (options.author === undefined || options.author === '') {
      console.error(
        'Error: Author ID is required. Set --author or FIELDBOOKS_AUTHOR_ID environment variable.',
      );
      process.exit(1);
    }
